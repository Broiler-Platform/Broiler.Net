...
test('a raised floor keeps numbers spent outside nuget.org from being reused', () => {
  // Broiler.Net's floor is preview.1. Siblings record previews spent on the retired
  // GitHub Packages feed (here preview.1..3) as a preview.4 floor; nuget.org may lag.
...
test('a package ID the feed has never seen starts at the configured floor', async () => {
  // Broiler.Net's first release: nuget.org answers 404 for the new ID, so preview.1 is used.
  const versions = await readVersions('https://feed/index.json', ['Broiler.Net'], {}, fakeFeed({
    'https://feed/flat/broiler.net/index.json': 404,
  }));
  assert.deepEqual(versions, []);
  assert.equal(chooseVersion('0.1.0-preview.1', versions), '0.1.0-preview.1');
  assert.equal(chooseVersion('0.1.0-preview.1', versions, { tag: 'v0.1.0-preview.1' }), '0.1.0-preview.1');
});
